...
export default function AssignmentSubmitForm(props: AssignmentSubmitFormProps) {
  ...
  async function handleSubmit() {
    if (!selectedAssignmentId) {
      setWarnMessage('Please select an assignment.');
      return;
    }
    setSubmitting(true);
    setSuccessMessage('');
      ...
      >
        ...
        <div className="overflow-x-scroll text-ellipsis overflow-hidden pb-3">
          {successMessage ? (
            ...
            <span className="text-warning">{warnMessage}</span>
          ) : errorMessage ? (
